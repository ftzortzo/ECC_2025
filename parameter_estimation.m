function [IDM_a,IDM_b] = parameter_estimation(leader_position,leader_velocity,follower_position,follower_velocity,k_leader,k_follower,llead,dt)
    %% Filippos Notes
    ...
    try
        ...
        u = [leader_position(1:k),leader_velocity(1:k)];
        ...
        P(2) = struct('Name','T_h','Unit','s','Value',1.6,'Minimum',0.5,'Maximum',4,'Fixed',true);
        ...
        P(6) = struct('Name','delta','Unit','','Value',4.0,'Minimum',2,'Maximum',6,'Fixed',true);
        ...
        m0 = idnlgrey('idm_dt_nl', order, P, x0, dt, 'Name','IDM_dt'); % I was trying to use @idm_dt_nl_local but seemed to not work...?
        m0 = setinit(m0,'Fixed',[false; false]);
        opt = nlgreyestOptions('Display','off');
        opt.SearchMethod = 'gn'; %gaussian search method
        opt.SearchOption.MaxIterations = 10;
        opt.SearchOption.Tolerance = 1e-3; %stopping loss function thd for gaussian to try and tune perf vs acc%
        opt.OutputWeight = [];
...
function [xnext, y] = idm_dt_nl_local(~, x, u, v0, T_h, s0, a, b, delta, llead, Ts, varargin)
    ...
    acc = idm_dvdt_local(vE, max(s,1e-3), dv, p);
...
function acc = idm_dvdt_local(v_a, s_a, dv_a, p)
